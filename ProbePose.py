...
I1 = target.natsort(glob.glob(os.path.join(base,config['right_folder'],'*')))
I2 = target.natsort(glob.glob(os.path.join(base,config['left_folder'],'*')))

# Load stereo calibration parameters
params_file = config['params_file']
if params_file.split('.')[1] == 'npz':
    Params = np.load(os.path.join(os.path.dirname(base),params_file))
elif params_file.split('.')[1] == 'mat':
    Params = sio.loadmat(os.path.join(os.path.dirname(base),params_file))
...
print('Mean absolute error of distances: {}'.format(np.array(errs).mean(0)))
